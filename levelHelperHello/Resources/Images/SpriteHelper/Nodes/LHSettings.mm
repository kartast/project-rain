...
-(void)setActiveContactNode:(LHContactNode*)node{
    
    if(contactNode)
        return; //we wont register another contact node if we already have one
    
    contactNode = node;
}
-(LHContactNode*)activeContactNode{
    return contactNode;
}
...
{
//    NSLog(@"WE HAVE FOUND DEVICE %d", device);
